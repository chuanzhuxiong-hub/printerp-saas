...
export default async function HelpPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <Link href="/app/help/getting-started" className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">第一次使用 PrintERP</Link>
      </PageHeader>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {helpCategories.map((category, index) => (
          <Link key={category.href} href={category.href} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50/40">
            ...
            <span className="mt-4 inline-block text-sm font-semibold text-brand">查看教程</span>
          </Link>
        ))}
      </section>
    </div>
