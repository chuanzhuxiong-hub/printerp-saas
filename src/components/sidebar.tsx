...
export function Sidebar({
  ...
  return (
    <aside className={cn("sticky top-0 flex h-screen shrink-0 flex-col border-r border-slate-200 bg-white text-slate-700 shadow-sm transition-all", collapsed ? "w-20" : "w-72")}>
      ...
      <div className="border-t border-slate-200 p-3">
        <div className={cn("mb-3 flex items-center gap-3 rounded-xl bg-slate-50 p-3", collapsed && "justify-center")}>
        ...
        {maintenanceMode ? (
          <a href="/admin" className="block rounded-lg px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">{collapsed ? "后台" : "返回平台后台"}</a>
        ) : (
          <form action="/api/auth/logout" method="post">
            <button className="w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-slate-600 hover:bg-slate-50">{collapsed ? "退出" : "退出登录"}</button>
          </form>
        )}
      </div>
    </aside>
